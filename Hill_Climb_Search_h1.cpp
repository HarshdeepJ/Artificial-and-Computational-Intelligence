...
/*Calculate heuristic function calculates the number of misplaced tiles and returns it*/
...
public:
    ...
    /*Checks all the possible next states and adds them into the children vector of the present node if and
    only if the heuristic of the children node is less than that of parent*/
    void what_next(node *parent)
    {
        int pos_of_B_x = -1, pos_of_B_y = -1;
        int temp_x = INT_MAX, temp_y = INT_MAX;
        int heur = INT_MAX;
        for (int i = 0; i < SIZE_OF_MAT; i++)
        {
            for (int j = 0; j < SIZE_OF_MAT; j++)
        ...
        for (int i = -1; i <= 1; i += 2)
        {
            for (int j = 1; j <= 2; j++)
            {
                ...
                if (pos_of_B_x + i * j / 2 < SIZE_OF_MAT && pos_of_B_y + i * j % 2 < SIZE_OF_MAT && pos_of_B_y + i * j % 2 >= 0 && pos_of_B_x + i * j / 2 >= 0)
                {
                    ...
                    if (heur>calculate_heuristic(temp_matrix))
                    {
                        heur = calculate_heuristic(temp_matrix);
                        temp_x = to_swap_x;
                        temp_y = to_swap_y;
                    }
                }
        ...
        if (heur<calculate_heuristic(parent->matrix))
        {
            string temp_matrix[SIZE_OF_MAT][SIZE_OF_MAT];
                for (int k = 0; k < SIZE_OF_MAT; k++)
                {
            ...
            swap(temp_matrix[pos_of_B_x][pos_of_B_y], temp_matrix[temp_x][temp_y]);
...
{
    int count = 0;
    cout<<"Output will be written in 'Output Hill_Climb_Search_h1.txt'"<<endl;
    freopen("Output Hill_Climb_Search_h1.txt", "w", stdout);
